...
class KAISTPed(data.Dataset):
    ...
    def __init__(self, args, condition='train'):
        ...
        elif self.dataset_type == "CVC14":
            self._imgpath = os.path.join('%s', '%s', '%s', 'Train', '%s', '%s.jpg')
    ...
    def pull_item(self, index):
        ...
        if self.dataset_type == "KAIST":
            ...
            vis = Image.open( self._imgpath % ( *frame_id[:-1], set_id, 'Visible', vid_id, img_id ))
        ...
        if self.mode == 'train': 
            ...
            if self.dataset_type == "KAIST":
                vis_boxes_path = self._annopath % ( *frame_id[:-1], set_id, vid_id, 'visible', img_id )
                lwir_boxes_path = self._annopath % ( *frame_id[:-1], set_id, vid_id, 'lwir', img_id)
            elif self.dataset_type == "CVC14":
                vis_boxes_path = self._annopath % ( *frame_id[:-1], set_id, 'Visible', img_id )
                lwir_boxes_path = self._annopath % ( *frame_id[:-1], set_id, 'FIR', img_id )

            for line in open(vis_boxes_path) :
                vis_boxes.append(line.strip().split(' '))
            for line in open(lwir_boxes_path) :
...
class KAISTPedWSBatch(KAISTPedWSEpoch):
    ...
    def __init__(self, args, condition='train', sample = None):
        ...
        if self.dataset_type == "KAIST":
            self.load_txt(self, sample, '../data/kaist-rgbt')
            self._imgpath = os.path.join('%s', 'images', '%s', '%s', '%s', '%s.jpg')
            self._annopath = os.path.join('%s', 'annotations_paired', '%s', '%s', '%s', '%s.txt')
        elif self.dataset_type == "CVC14":
            self.load_txt(self, sample, '../data/CVC-14')
            self._imgpath = os.path.join('%s', '%s', '%s', 'Train', '%s', '%s.jpg')
            self._annopath = os.path.join('%s', '%s', '%s', 'Train', 'Annotations', '%s.txt')

    def load_txt(self, sample, root_path):
        if sample == "Labeled":
            image_set = self.L_image_set
        elif sample == "Unlabeled":
            image_set = self.U_image_set
        else:
            image_set = self.image_set
        self.ids = list()
        for line in open(os.path.join('./imageSets', image_set)):
            self.ids.append((root_path, line.strip().split('/')))
