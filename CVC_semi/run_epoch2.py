...
from model import SSD300, SSD300_3Way, MultiBoxLoss, BoxLoss
...
def softTeaching_every_batch(s_model: SSD300_3Way,
    ...
    for (batch_idx, (wL_image_vis, wL_image_lwir, sL_image_vis, sL_image_lwir, L_boxes, L_labels, L_is_anno)), \
        ...
        with torch.no_grad():
            ...
            # inference
            locs, scores, features = t_model(w_image_vis, w_image_lwir)

            # get unlabeled inference results
            locs, scores = locs[len(L_boxes):], scores[len(L_boxes):]
        ...
        lwir_student_locs, lwir_student_scores, _ = s_model(s_image_vis, s_image_lwir)  # (N, 8732, 4), (N, 8732, n_classes)
        ...
        for floc, fscore, vloc, vscore, lloc, lscore, box, label in zip(fusion_un_locs, fusion_un_scores, vis_un_locs, vis_un_scores, \
                                                                        lwir_un_locs, lwir_un_scores, pseudo_boxes, pseudo_labels):
        ...
        GAP_weight =  calc_weight_by_GAPVector_distance(features, L_boxes, id_box_dict, len(L_boxes), config.test.input_size)
        ...
        # loss = fusion_sup_loss + GAP_weight * (fusion_un_loss + vis_un_loss + lwir_un_loss)
        loss = fusion_sup_loss + vis_sup_loss + lwir_sup_loss + GAP_weight * fusion_un_loss
        ...
        if batch_idx % kwargs.get('print_freq', 10) == 0:
            logger.info('Iteration: [{0}/{1}]\t'
                        ...
                        'num of Positive {vis_sup_n_positives} {lwir_sup_n_positives} {fusion_sup_n_positives} '
                        '{vis_un_n_positives} {lwir_un_n_positives} {fusion_un_n_positives}\t'.format(
                                                              batch_idx, 
                                                              len(U_dataloader),
