...
class KAISTPedWS(KAISTPed):
    ...
    Arguments:
        ...
        condition (string, optional): load condition
            (default: 'Reasonabel')
    """
    def __init__(self, args, aug_mode, condition='train', propname=None):
    ...
    #                self.annotations[img_id].append([float(x), float(y), float(w), float(h), float(score)])
        ...
        # Load flip probs
        #self.props = dict()
        #self.props_name = os.path.join(jobs_dir, f'props_{epoch:3d}.txt')
        #with open(self.propname, "r") as f:
    ...
    def pull_item(self, index):
        ...
        set_id, vid_id, img_id = frame_id[-1]
         ...
         #   with open(self.propname, "a") as f:
