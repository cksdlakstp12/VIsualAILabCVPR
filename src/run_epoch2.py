...
def softTeaching_every_batch(s_model: SSD300,
    ...
    # Batches
    for (batch_idx_L, (wL_image_vis, wL_image_lwir, sL_image_vis, sL_image_lwir, L_boxes, L_labels, _, L_is_anno)), (batch_idx_U, (wU_image_vis, wU_image_lwir, sU_image_vis, sU_image_lwir, U_boxes, U_labels, _, U_is_anno)) in zip(enumerate(L_dataloader), enumerate(U_dataloader)):
        ...
        for anno, b, l, pl, ps in zip(is_anno, boxes, labels, predicted_locs, predicted_scores):
            if anno:
                sup_boxes.append(b.to(device))
                sup_labels.append(l.to(device))
                ...
                un_boxes.append(b.to(device))
                un_labels.append(l.to(device))
        ...
        if len(un_boxes) > 0:
            ...
            un_loss, un_cls_loss, un_loc_loss, un_vis_n_positives = criterion(un_predicted_locs, un_predicted_scores, un_boxes, un_labels)
        ...
        if batch_idx % kwargs.get('print_freq', 10) == 0:
            logger.info('Iteration: [{0}/{1}]\t'
                        ...
                        #'is_anno {is_anno}\t'
                        'num of Positive {sup_vis_n_positives} {sup_lwir_n_positives} {un_vis_n_positives} {un_lwir_n_positives}\t'.format(batch_idx, len(U_dataloader),
