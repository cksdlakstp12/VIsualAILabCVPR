...
def softTeaching_every_iter(s_model: SSD300,
    ...
    # Batches
    for (batch_idx_L, (wL_image_vis, wL_image_lwir, sL_image_vis, sL_image_lwir, at_wL_image_vis, at_wL_image_lwir, at_sL_image_vis, at_sL_image_lwir, L_boxes, L_labels, _, L_is_anno,L_cluster_id)), (batch_idx_U, (wU_image_vis, wU_image_lwir, sU_image_vis, sU_image_lwir, at_wU_image_vis, at_wU_image_lwir, at_sU_image_vis, at_sU_image_lwir, U_boxes, U_labels, _, U_is_anno,U_cluster_id)) in zip(enumerate(L_dataloader), enumerate(U_dataloader)):
        ...
        at_s_image_vis = torch.cat((at_sL_image_vis, at_sU_image_vis), 0)
        at_s_image_lwir = torch.cat((at_sL_image_lwir, at_sU_image_lwir), 0)
        ...
        rgb_at_predicted_locs, rgb_at_predicted_scores = t_model(at_w_image_vis, w_image_lwir)
        lwir_at_predicted_locs, lwir_at_predicted_scores = t_model(w_image_vis, at_w_image_lwir)

        #filtered_rgb_locs_demo, filtered_rgb_scores_demo, filtered_lwir_locs_demo, filtered_lwir_scores_demo = filter_consistent_entries(rgb_at_predicted_locs, rgb_at_predicted_scores, lwir_at_predicted_locs, lwir_at_predicted_scores, threshold=0.5)
        ...
        rgb_at_predicted_locs_non, rgb_at_predicted_scores_non = rgb_at_predicted_locs[anno_index], rgb_at_predicted_scores[anno_index]
        lwir_at_predicted_locs_non, lwir_at_predicted_scores_non = lwir_at_predicted_locs[anno_index], lwir_at_predicted_scores[anno_index]
        ...
        rgb_at_detections = t_model.module.detect_objects(rgb_at_predicted_locs_non, rgb_at_predicted_scores_non,
                                                          min_score=0.1, max_overlap=0.425, top_k=200)
        lwir_at_detections = t_model.module.detect_objects(lwir_at_predicted_locs_non, lwir_at_predicted_scores_non,
                                                              min_score=0.1, max_overlap=0.425, top_k=200)
                    ...
                    for key_j in temp_box:
                        for box_i in at_rgb_temp_box[key_i]:
                            for box_j in temp_box[key_j]:
                                ...
                                iou = compute_iou(box_i_corners, box_j_corners)
        ...
        rgb_boxes, rgb_labels = return_boxes_labels(rgb_final_box, anno_index, rgb_boxes, rgb_labels)
        ir_boxes, ir_labels = return_boxes_labels(lwir_final_box, anno_index, ir_boxes, ir_labels)
        ...
        #+ mse_loss(rgb_un_cls_loss.float(), lwir_un_cls_loss.float()) + mse_loss(rgb_un_loc_loss.float(), lwir_un_loc_loss.float())

       # print(f"sup_loss : {sup_loss}, rgb_un_loss : {rgb_un_loss}, lwir_un_loss : {lwir_un_loss}, cls_MSE_loss : {mse_loss(rgb_un_cls_loss.float(), lwir_un_cls_loss.float())}, loc_MSE_loss : {mse_loss(rgb_un_loc_loss.float(), lwir_un_loc_loss.float())}")
