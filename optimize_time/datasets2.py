...
class KAISTPedWSBatch(KAISTPedWSEpoch):
    ...
    def __getitem__(self, index):
        ...
        w_vis, w_lwir, s_vis, s_lwir, at_w_vis, at_w_lwir, boxes, labels, is_annotation, cluster_id = self.pull_item(index)
        ##print(self.ids[index])
        ##print(vis, lwir, boxes, labels, torch.ones(1,dtype=torch.int)*index, self.ids[index])
        return w_vis, w_lwir, s_vis, s_lwir, at_w_vis, at_w_lwir, boxes, labels, is_annotation, cluster_id
    ...
    def apply_saliency_map(self, image, saliency):
       ...
       # print(*frame_id[:-1],"123" ,set_id, "123", vid_id, "123", 'visible', "123", img_id)
        ...
        if self.mode == 'train':
            ...
            if id in self.teacher_image_ids.values():
                ...
                saliency_map_w_vis = self.create_saliency_map(w_vis, is_rgb=True)
                # saliency_map_s_vis = self.create_saliency_map(s_vis, is_rgb=True)
                saliency_map_w_lwir = self.create_saliency_map(w_lwir, is_rgb=False)
                # saliency_map_s_lwir = self.create_saliency_map(s_lwir, is_rgb=False)
                
                at_w_vis = self.apply_saliency_map(w_vis, saliency_map_w_vis)
                # at_s_vis = self.apply_saliency_map(s_vis, saliency_map_s_vis)
                at_w_lwir = self.apply_saliency_map(w_lwir, saliency_map_w_lwir)
                # at_s_lwir = self.apply_saliency_map(s_lwir, saliency_map_s_lwir)
                ...
                at_w_vis, at_w_lwir, _, _, _ = self.weak_transform(at_w_vis, at_w_lwir, None, None, self.pair)
                # at_s_vis, at_s_lwir, _, _, _ = self.strong_transform(at_s_vis, at_s_lwir, None, None, self.pair)

                #print(vis_boxes, lwir_boxes)
                # return w_vis, w_lwir, s_vis, s_lwir, at_w_vis, at_w_lwir, at_s_vis, at_s_lwir, None,    None,   is_annotation, cluster_id
                return w_vis, w_lwir, s_vis, s_lwir, at_w_vis, at_w_lwir, None, None, is_annotation, cluster_id
            else: 
                for line in open(self._annopath % ( *frame_id[:-1], set_id, vid_id, 'visible', img_id )) :
                    vis_boxes.append(line.strip().split(' '))
                for line in open(self._annopath % ( *frame_id[:-1], set_id, vid_id, 'lwir', img_id)) :
        ...
        self.strong_transform.update_prop(prop)
        ## Apply transforms
        #saliency_map_vis = self.create_saliency_map(vis, is_rgb=True)
        #saliency_map_lwir = self.create_saliency_map(lwir, is_rgb=False)
                
        #at_vis = self.apply_saliency_map(vis, saliency_map_vis)
        #at_lwir = self.apply_saliency_map(lwir, saliency_map_lwir)
        ...
        return vis, lwir, vis, lwir, vis, lwir, boxes_t, labels, is_annotation, cluster_id
    ...
    def collate_fn(self, batch):
        ...
        for b in batch:
            ...
            # s_lwir.append(b[3])
            # at_w_vis.append(b[4])
            # at_w_lwir.append(b[5])
            # at_s_vis.append(b[6])
            # at_s_lwir.append(b[7])
            # boxes.append(b[8])
            # labels.append(b[9])
            # index.append(b[10])
            # is_anno.append(b[11])
            # cluster_id.append(b[12])
            w_vis.append(b[0])
            w_lwir.append(b[1])
            s_vis.append(b[2])
            s_lwir.append(b[3])
            ...
            labels.append(b[7])
            is_anno.append(b[8])
            cluster_id.append(b[9])

        w_vis = torch.stack(w_vis, dim=0)
        w_lwir = torch.stack(w_lwir, dim=0)
        s_vis = torch.stack(s_vis, dim=0)
        s_lwir = torch.stack(s_lwir, dim=0)
        at_w_vis = torch.stack(at_w_vis, dim=0)
        at_w_lwir = torch.stack(at_w_lwir, dim=0)
        # at_s_vis = torch.stack(at_s_vis, dim=0)
        # at_s_lwir = torch.stack(at_s_lwir, dim=0)
  
        # return w_vis, w_lwir, s_vis, s_lwir, at_w_vis, at_w_lwir, at_s_vis, at_s_lwir, boxes, labels, index, is_anno, cluster_id
        return w_vis, w_lwir, s_vis, s_lwir, at_w_vis, at_w_lwir, boxes, labels, is_anno, cluster_id
...
#             (default: 'Reasonabel')
#     """
#     def __init__(self, args, sample=None, condition='train'):
#         super().__init__(args, "strong", condition)
#         self.weak_transform = args[condition].batch_weak_transform 
#         self.strong_transform = args[condition].batch_strong_transform 
#         self.baseline_transform = args[condition].baseline_transform
...
#         assert sample is not None, "sample parameter should not be None"
#         if sample == "Labeled":
#             self.ids = list()
#             for line in open(os.path.join('./imageSets', self.L_image_set)):
#                 self.ids.append(('../data/kaist-rgbt/', line.strip().split('/')))
...
#         ##print(self.ids[index])
#         ##print(vis, lwir, boxes, labels, torch.ones(1,dtype=torch.int)*index, self.ids[index])
#         return w_vis, w_lwir, s_vis, s_lwir, boxes, labels, torch.ones(1,dtype=torch.int)*index, is_annotation
...
#         return w_vis, w_lwir, s_vis, s_lwir, boxes, labels, index, is_anno
