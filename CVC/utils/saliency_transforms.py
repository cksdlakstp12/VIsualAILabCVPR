...
class SaliencyAugmentation(object):
    ...
    def __call__(self, img, mask=None, box_vis=None, box_lwir=None, pair=None):
        if random.random() < self.prob:
            saliency_map_vis = self.create_saliency_map(img, is_rgb=True)
            saliency_map_lwir = self.create_saliency_map(mask, is_rgb=False)
            img = self.apply_saliency_map(img, saliency_map_vis)
            mask = self.apply_saliency_map(mask, saliency_map_lwir)
...
class FusionDeadZone(object):
    ...
    def __call__(self, vis, lwir, box_vis=None, box_lwir=None):
        ...
        """
        #print("\n호출호출호출\n")
            #print("before vis, lwir :", box_vis, box_lwir)
            #print("before img, ir", vis, lwir)
        vis = self.func_R(vis)
        lwir = self.func_T(lwir)
        box_vis = None
            #print("after vis, lwir :", box_vis, box_lwir)
            #print("after img, ir", vis, lwir)
...
class TT_FaultTolerant(object):
    def __init__(self, prob=[0.9, 0.5, [0.25,0.5,0.75]]):
        self.prob = prob
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, img, ann=None, box_vis=None, box_lwir=None, pair=None):
        ...
        if random.random() < self.p:
            img = F.hflip(img) if img is not None else print("이거도 되나?11")
...
class RandomResizedCrop(object):
    ...
    def __call__(self, img, mask=None, box_vis=None, box_lwir=None, pair=None):
        ...
        """
        if random.random() < self.p:
            i, j, h, w, valid_box = self.get_params(img, self.scale, self.ratio, box_lwir)
        
            width, height = img.size        

            img = F.resized_crop(img, i, j, h, w, self.size, self.interpolation) if img is not None else print("이거도 되나?22")  
